...
def writeItem(item):
    ...
    writeLine(item_file, [itemID, name, currently, buy_price, first_bid, started, ends, sellerID, description, number_of_bids])
...
def parseXml(f):
    ...
    document = getElementByTagNameNR(dom, 'Items')
    item_list = getElementsByTagNameNR(document, 'Item')
    ...
    for item in item_list:
        ...
        seller = item.getElementsByTagName('Seller')[0]
        seller_name = seller.getAttribute("UserID")
        try:
            index = sellers.index(seller_name)
        except ValueError:
            sellers.append(seller_name)
            writeSeller(item)

        bids_grp = getElementByTagNameNR(item, 'Bids')
        bid_list = getElementsByTagNameNR(bids_grp, 'Bid')
        for bid in bid_list:
            writeBid(bid, itemID(item))
            bidder_id = bidderID(bid)
            bidder_rating = bidderRating(bid)
            try:
                index = sellers.index(bidder_id)
            except ValueError:
...
def main(argv):
    if len(argv) < 2:
        print >> sys.stderr, 'Usage: python super_xml_parser.py <path to xml files>'
        sys.exit(1)
